...
function (_Component) {
  ...
  function TodoList() {
    ...
    _defineProperty(_assertThisInitialized(_assertThisInitialized(_this)), "isChecked", function (itemId) {
      // const itemValue = this.state.items.filter((item)=>itemId===item.id)
      // const newItems = {...itemValue[0],isCheckedVal:true}
      _this.state.items.map(function (item, i) {
        if (item.id === itemId) {
      ...
      _this.setState({
        items: _toConsumableArray(_this.state.items).concat([newItems])
      }, function () {
...
//# sourceMappingURL=index.js.898e125f07ace1532d82.hot-update.js.map